...
/// dst[pos]<-bm
void copy_bitmap_to(Rng<U8> dst, Arr<I32,2> ddim, Arr<I32,2> pos, FT_Bitmap const& bm){
	print("INFO: copy_bitmap_to: {} {} {} {}\n",
		pos[0], pos[1], I32(bm.width), I32(bm.rows)
	);
	A(I32(bm.pixel_mode)==FT_PIXEL_MODE_GRAY);
	A(I32(bm.palette_mode)==0);
...
int main(int argc, char * argv[]){
	...
	// open rc file
	auto name_rc=format(tmp.rng(), "output/{}.rc", fontname);
	print("INFO: file_rc: {}\n", name_rc);
    ...
    */
	...
	I32 f_asc = m.ascender >> 6;
	I32 f_desc = m.descender >> 6;
	I32 f_height = m.height >> 6;
	I32 f_width = f_height / 2;
	if (verbose>0){
		print("ascender {}\n", f_asc);
		print("descender {}\n", f_desc);
		print("height {}\n", f_height);
	...
	// out image
	//I32 xx=pixsize*16, yy=pixsize*16;
	I32 const xx=1024, yy=1024;
	...
	for (auto charcode: chars){
		...
		if (bm.width>0 and bm.rows>0){
			curr_line_height = std::max(curr_line_height, I32(bm.rows));
		...
		// rc file
		print(&rc_file, "sprite {} ", I32(charcode));
		...
		print(&rc_file, "box {} {} {} {} ",
			gly_pos[0], gly_pos[1], I32(bm.width), I32(bm.rows)
		);
		print(&rc_file, "rbox {} {} {} {} ",
			I32(gly->bitmap_left), I32(-gly->bitmap_top + f_asc),
			I32(bm.width), I32(bm.rows)
		);
		print(&rc_file, "adv {}\n", I32(gly->advance.x >> 6));
	...
	rc_file.close();
	// save png
	auto name_png=format(tmp.rng(), "output/{}.png", fontname);
	print("INFO: write output image: {}\n", name_png);
